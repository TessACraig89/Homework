...
**Build a web-based, CRUD application using Python and Django.**

## Presentations will be Thurs Feb 22 after Lunch

## Objectives

### Build an app in your second language, and collaborate as a team!.
...
- **HTML / Django Templates**: Your HTML should be semantic and valid. Your app uses Django templates to render information on the page.
- **Ptyon and Django**: Your app will need to have its own server, built using Django.
- **MVC Pattern**: Your app uses Models, Views, Controllers pattern we have gone over in class.
- **Postgres**: Your app will need to persist data. Your app should have at least two database tables. The second DB table can be post-MVP.
...
- **At least 2 Resources**: Should have at least one relationship between these resources, such as a User has many Posts
- **Auth**: Take advantage of Django's built in Auth
- **A third party API**:  like Twilio, or OMBD, Google Maps, etc.
...
- For this project we want you to use [Trello](trello.com). Give us a link to a trello board with all of your User Stories as items on the Trello board. Your collumns should be Todo, In Progress, and Complete. [Here's a good resource on using Trello for Kanban](https://blog.hubstaff.com/kanban-with-trello/)
  - Please make sure the trello board is public so we can see it
...
- A presentation in class that:
    ...
    - Each member of the group should talk about what they worked on
